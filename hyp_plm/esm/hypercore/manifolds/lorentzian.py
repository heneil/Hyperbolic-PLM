...
class Lorentz(LorentzOri):
    ...
    def oxy_angle(self, x, y, eps: float = 1e-5, c=None):
        """
        ...
        acos_input = acos_numer / ((torch.norm(x_space, dim=-1) * acos_denom).clamp_min(eps))
        _angle = torch.acos(torch.clamp(acos_input, min=-1 + eps, max=1 - eps))
        return _angle

    def half_aperture(self, x, min_radius: float = 0.1, eps: float = 1e-5, c=None):
        """
        ...
        if c is not None:
            asin_input = 2 * min_radius / ((torch.norm(x[..., 1:], dim=-1) * (1/c)**0.5).clamp_min(eps))
        else:
            asin_input = 2 * min_radius / ((torch.norm(x[..., 1:], dim=-1) * (1/self.c)**0.5).clamp_min(eps))
        _half_aperture = torch.asin(torch.clamp(asin_input, min=-1 + eps, max=1 - eps))
        return _half_aperture

    def oxy_angle_pairwise(self, X, Y, K: float = 0.1, eps: float = 1e-4, c=None):
        x_t, x_sp = X[..., 0], X[..., 1:]     
        y_t, y_sp = Y[..., 0], Y[..., 1:]   
        c_inv = self.c if c is None else c      
        c_act = 1.0 / c_inv                     
        space_dot = x_sp @ y_sp.transpose(0, 1)                     
        inner_L = (-x_t[:, None] * y_t[None, :]) + space_dot      
        c_xyl = c_act * inner_L                                   
        x_norm = x_sp.norm(dim=-1)                                  
        num = y_t[None, :] + x_t[:, None] * c_xyl             
        den_sq = (c_xyl ** 2) - 1.0
        den = (x_norm[:, None]) * torch.sqrt(torch.clamp(den_sq, min=eps)) 

        cos_ext = torch.clamp(num / (den + eps), min=-1.0 + eps, max=1.0 - eps)
        return torch.acos(cos_ext)                                 

    def half_aperture_row(self, X, K: float = 0.1, eps: float = 1e-4, c=None):
        x_sp = X[..., 1:]
        c_inv = self.c if c is None else c
        c_act = 1.0 / c_inv
        x_norm = x_sp.norm(dim=-1)                              
        aper_arg = (2.0 * K) / ((c_act ** 0.5) * x_norm + eps)    
        aper_arg = torch.clamp(aper_arg, 0.0, 1.0 - eps)
        return torch.asin(aper_arg)        

    def cdist(self, x: torch.Tensor, y: torch.Tensor):
            x = x.clone()
            x.narrow(-1, 0, 1).mul_(-1)
            return torch.acosh((-(1/sefl.c) * (x @ y.transpose(-1, -2))).clamp(min=1 + 1e-4)) / (1 / (self.c ** 2))                       
